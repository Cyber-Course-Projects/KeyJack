...
for x in range(len(data)):
  print(data[x])
  crc = crc_update(crc, ord(chr(data[x])))
...
with open(path) as f:
  ...
  payloads[0] = str(payloads[0][0:2]) + chr((ord(chr(payloads[0][2])) + 1)) + chr((ord(chr(payloads[0][3])) - 1)) + str(payloads[0][5:])

# Add the firmware CRC
payloads.append('\x20\x67\xFE\x02' + str(struct.pack('!H', crc)) + '\x00'*26)

# Instantiate the dongle
dongle = unifying.unifying_dongle()
